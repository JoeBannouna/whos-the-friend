...
import RoomView, {} from '../src/Views/RoomView.js';
...
function verifyObject(obj1, obj2, silentSuccess = false) {
    try {
...
function assertDefined(value, label, silentSuccess = false) {
...
const mainRoomGameCycleTest = async () => {
    ...
    const initialCategoryQuestions = jsonQuestions.questions.filter(q => q.cat == initialCategory.id);
    ...
    const voteForSelfResponse = await RoomView.updatePlayerVoteForCurrentQuestion(roomId, johnId, johnId);
    ...
    let announceWinnersResponse = null;
    let acceptVotesResponse = null;
    for (let i = 0; i < expectedBroadcastMessage.currentCategory.questions.length - 1; i++) {
        ...
        if (!isLastLoop)
            check(() => acceptVotesResponse.status === 'accepting_votes', true);
    }
    ...
    let advanceToNextCategory = null;
    // now we finish off all the other categories and finish the game
    for (let k = 1; k < jsonQuestions.categories.length; k++) {
        const currJsonQuestions = jsonQuestions.questions.filter(q => q.cat == jsonQuestions.categories[k].id);
        let announceWinnersResponseLoop = null;
        let acceptVotesResponseLoop = null;
        for (let i = 0; i < currJsonQuestions.length; i++) {
            ...
            if (!isLastLoop)
                check(() => acceptVotesResponseLoop.status === 'accepting_votes', true);
        }
...
const tests = [mainRoomGameCycleTest];
