...
app.get('/rooms/player/:roomId/:playerId', async (req, res) => {
    const response = await RoomView.playerExists(req.params.roomId, req.params.playerId);
    if (response == false)
        res.status(400).send();
    else
        res.status(200).send();
...
app.post('/rooms/create', async (req, res) => {
    ...
    if (validation.success == false)
        res.status(400).send({ msg: validation.msg });
    else {
        const response = await RoomView.createRoom(req.body.roomName, req.body.roomPass);
        if (response === null)
            res.status(500).send({ msg: 'failed to create a room' });
        else
            res.status(200).send();
    }
});
