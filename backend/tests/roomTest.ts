...
function depthConsoleLog(obj: any) {
...
const mainRoomGameCycleTest = async () => {
  ...
  check(
    () =>
      room.gameResults.categoryPodiums.length === 1 &&
  ...
  check(
    () =>
      acceptVotesResponse.gameResults.questionPodiums.length ==
  ...
  check(
    () => announceGameWinner.gameResults.categoryPodiums.length === jsonQuestions.categories.length
  );
